...
// If mocks are enabled via env var, export the mock api
const USE_MOCKS = (process.env.NEXT_PUBLIC_USE_MOCKS || 'false') === 'true';

// Declare exports to satisfy TypeScript at top-level
export let api: {
...
if (USE_MOCKS) {
    api = mockApi.api;
    defaultClient = axios.create(); // noop client to satisfy imports
} else {
        ...
        async (error: AxiosError<ApiError>) => {
            ...
            if (error.response?.status === 401 && !originalRequest._retry) {
                ...
                try {
                    ...
                    if (refreshToken) {
                        const response = await axios.post(`${env.API_URL}/v1/auth/refresh`, {
                            refreshToken,
                        });
